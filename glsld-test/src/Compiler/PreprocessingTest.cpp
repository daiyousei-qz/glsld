...
TEST_CASE_METHOD(CompilerTestFixture, "Compiler::PreprocessingTest")
{
    ...
    SECTION("Include")
    {
        ...
        SECTION("SourceFileOverlay")
        {
            ...
            REQUIRE(sourceVfs);
            REQUIRE(sourceVfs->AddFile("mem:/shader/main.glsl", R"(
                #include "include/header.glsl"
                HEADER_VALUE
            )")
                        .has_value());
            ...
            REQUIRE(includeVfs);
            REQUIRE(includeVfs->AddFile("mem:/shader/include/header.glsl", R"(
                #define HEADER_VALUE fallback_token
            )")
                        .has_value());

            auto vfs = OverlayFileSystem::Create(
                std::vector<std::shared_ptr<VirtualFileSystem>>{sourceVfs, includeVfs});
            REQUIRE(vfs);
            ...
            REQUIRE(result != nullptr);
            REQUIRE(callback.resolvedUris == std::vector<std::string>{"mem:/shader/include/header.glsl"});
            CheckTokens(result->GetUserFileArtifacts().GetTokens(), {IdTok("fallback_token"), EofTok()});
        }
