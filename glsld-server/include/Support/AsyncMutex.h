...
namespace glsld
{
    ...
    private:
        ...
        private:
            ...
            public:
                OperationState(AsyncMutex& mutex, Receiver&& rcvr)
                    : mutex(&mutex), receiver(std::forward<Receiver>(rcvr))
                {
                ...
                {
                    std::unique_lock lock{mutex->internalMutex};
                    if (!mutex->locked) {
                        mutex->locked = true;
                        lock.unlock();
                        resume(*this, false);
                        ...
                        mutex->signalResumeQueue.push(this);
                    }
                }
            };
    ...
    public:
        ...
        auto Unlock() noexcept -> void
        {
            ...
            if (auto resumeWaiter = signalResumeQueue.pop(); resumeWaiter) {
